...
### 6-4. 공금 예산은 "통화별 주머니" (14단계)
- 출발 전에 이미 환전을 해오므로, 예산은 그 통화로 **고정**이다. `budgets.currency`(마이그레이션 0004)가 그 통화를 들고, 잔여는 환율을 전혀 쓰지 않고 `예산 − 사용`으로만 낸다.
- 공금 지출이 어느 주머니에서 빠지는지: **그 통화 주머니가 있으면 원금 그대로 거기서**, 없으면 `entries.krw`(저장 시점 환율로 이미 환산된 값)를 **원화 주머니**에서. 두 번째 규칙 덕분에 예산이 전부 원화인 기존 여행은 동작이 그대로다 — 이게 없으면 원화 주머니가 안 줄고 외화 주머니만 음수가 된다.
- 개인 결제는 각자 돈이라 주머니를 축내지 않는다. 단 `computeTotals`는 `perMember`에 없는 `member_id`를 공금으로 접으므로, 비활성 멤버 지출이 공금으로 새는 기존 버그가 주머니에도 그대로 영향을 준다(참여자 관리 작업에서 같이 고칠 것).
- `Totals.budget/remain/pct/remainCny` 단일 필드는 없어지고 `Totals.pots: Pot[]`로 대체됐다.
...
- [x] 14. (13단계 이후 추가) 공금 예산 통화 고정 — "이미 환전을 한 돈인데 환율이 바뀔 때마다 잔여 외화가 출렁인다"는 문제. **마이그레이션 0004**로 `budgets.currency`(기본값 `'KRW'`)를 추가하고 예산을 통화별 주머니로 다룬다(6-4 참고). `computeTotals`가 `budgets` 배열을 직접 받아 `pots: Pot[]`(통화/예산/사용/잔여/사용률)를 내고, 헤더·기록·내역·설정이 모두 주머니별로 표시한다. 설정 탭 예산 추가 폼에 통화 칩을 달았다(기본값은 목적지 나라 통화). `format.money(n, code)`로 원화/외화 표기를 한 군데로 모았다. 기존 여행 회귀(예산 원화 + CNY 지출 → 잔여 ₩327,800 동일), 환율을 바꿔도 TWD 잔여 불변, 주머니 둘, 주머니 없는 통화 폴백, 개인 결제 제외를 모두 검증 완료.
