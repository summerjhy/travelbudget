...
- [x] 14. (13단계 이후 추가) 관리자 세션 + 이모지 확대 — 관리자가 여행 코드(=여행 시작일 8자리)를 다 기억하지 못하는 문제. 초기 화면의 "관리자이신가요? 새 여행 만들기"를 **"관리자이신가요?"**로 바꾸고, 누르면 비밀번호만 받는 `AdminGate` → 세 갈래 `AdminMenu`(새 여행 만들기 / 여행 둘러보기 / 여행 관리하기)로 이어진다. 비밀번호는 `AdminContext`의 **메모리에만** 두고 `localStorage`에는 넣지 않는다 — 여행을 지울 수 있는 값이라 기기를 잃으면 곤란하다(새로고침하면 다시 묻는다). 검증은 `listTrips`를 실제로 호출해 401 여부로 판단하므로 클라이언트에 비밀번호가 하드코딩되지 않는다. `AdminBrowse`는 코드를 몰라도 진행중·준비중·종료 3그룹에서 아무 여행이나 열 수 있다("내가 만든 앱이니 모든 여행에 내가 이미 참여자"라는 전제). `AdminConsole`은 자체 비밀번호 화면을 지우고 "🗂 여행 관리하기"로 개칭. 이모지는 32→84개로 늘리고 `Intl.Segmenter` 기반 직접 입력칸을 붙여(자소 단위로 끊어 국기·피부색 결합 이모지도 한 글자로 처리) 목록에 없는 것도 쓸 수 있게 했고, 설정 탭에서 남의 이모지도 고칠 수 있다. "🔄 다른 여행"은 설정 탭 최하단으로 옮기고 홈 탭 안내 문구도 그에 맞췄다.
- [x] 15. (14단계 이후 추가) 최종 정산하기 + UI 다듬기 + 사진분석 안정화 — 아래 "15단계에서 확정된 구현 세부사항" 참고.

- [x] 16. (15단계 이후 추가) 첫 진입 온보딩 — 이름을 넣자마자 빈 입력창으로 떨어지던 흐름을 고쳤다. 홈 화면 설치 안내가 설정 탭(434줄) 맨 아래에 접혀 있어 처음 온 사람은 볼 일이 없었고, 그래서 브라우저 탭에서 쓰다가 다음에 또 카톡 링크를 찾아 들어와야 했다. 코드 → 이름 다음에 **딱 한 번** 끼어드는 `Onboarding`(설치 안내 + 사용법 3장)을 넣었다. 이미 홈 화면 앱(standalone)으로 열었으면 설치 단계는 통째로 건너뛴다(4단계 → 3단계). 플래그는 여행별이 아니라 **기기별**(`travelbudget.onboarded`) — 설치도 쓰는 법도 여행이 바뀐다고 다시 볼 이유가 없다. 설정 탭 꿀팁 맨 위에 "📖 사용법 다시 보기"를 두되, 플래그만 지우면 이미 마운트된 `Gate`의 state가 안 바뀌므로 새로고침으로 처음과 같은 경로를 태운다. 시스템 Chrome + Playwright로 android/ios/standalone 세 경우, 단계별 점 표시·건너뛰기·완료 후 재방문·설정 탭 재진입까지 검증 완료.
- **예산 삭제가 헤더에 반영 안 되던 버그 (17단계에서 수정)**: `useBudgets`를 5곳(헤더·홈·기록·내역·설정)에서 각각 호출하는데 훅 인스턴스마다 state 가 따로였다. 설정 탭에서 지워도 자기 목록만 바뀌고 **헤더(`TripLayout`)는 탭을 옮겨도 언마운트되지 않아** 낡은 총액이 새로고침 전까지 그대로 남았다. 게다가 `usePolling`은 `useEntries.refresh`만 돌려서 예산은 마운트 때 1회 조회 후 영영 갱신되지 않았다(남이 공금을 더 걷어도 안 보임). 여행별 공유 캐시 + `useSyncExternalStore` 구독으로 바꿔 한 곳에서 바뀌면 모든 화면이 같이 갱신되게 하고, `TripLayout`에 예산 폴링을 추가했다. `getSnapshot`이 매번 새 배열(`?? []`)을 만들면 무한 렌더가 나므로 빈 값은 고정 참조(`EMPTY`)를 쓴다.

- [x] 17. (16단계 이후 추가) 외화 예산의 두 성격 + 주머니 모델 — 가오슝 여행 헤더에 "17,499.68 TWD"라는 지갑에 있지도 않은 숫자가 떴다. 15,425 TWD를 45.38에 환전했는데, 화면이 원화 잔여(₩699,987)를 **오늘 시세 40으로 되나눠** 계산하고 있었다. 0006은 외화 예산을 전부 "미리 환전한 돈"으로 보고 원화(amount)를 진짜로 삼았지만, 외화 예산에는 성격이 다른 둘이 섞여 있다 — **① 미리 환전**(현금·트래블카드. 환전이 끝나 시세와 무관하게 고정) **② 실시간 환율**(계좌연동/신용카드. 결제할 때마다 그날 환율로 환산되는 한도). **두 경우 모두 외화 금액이 진짜이고 원화를 어떤 환율로 보여줄지만 다르다**(①은 환전 환율, ②는 그날 시세). **마이그레이션 0017**로 `budgets.prepaid`를 추가하고(기존 행은 전부 ①이라 기본값 true), `computeTotals`를 통화별 **주머니(Pot)** 모델로 바꿔 잔여를 외화에서 외화로 뺀다. 예산 입력 폼에 "미리 환전함 / 실시간 환율" 선택지를 넣었다. 헤더는 주머니별로 나란히 표시한다.
- [x] 18. (17단계 이후 추가) 공금 나눠주기 — 여행 중에 총무가 공금 일부를 멤버에게 미리 보내고 그 돈으로 결제하게 하는 일이 흔한데(상하이 여행에서 율 ₩700,000 · 혜연 ₩500,000), 이걸 기록할 데가 없어 **정산 방향이 통째로 뒤집혔다**. 미리 받아 결제한 사람이 "자기 돈을 썼다"고 잡히기 때문이다(앱은 혜연이 ₩484,267을 내야 한다고 했지만 실제로는 ₩215,733을 받아야 했다 — 그래서 별도로 Claude를 돌려 다시 정산했었다). **마이그레이션 0018**로 `fund_handouts`(받는 사람 / 방향 out·in / 금액+통화+환율 / 날짜 / 메모)를 만들고, 정산식에 `received(P)` 항을 더했다: `net(P) = fair − paidRaw − budget/n + received(P) + (총무면 budget − Σreceived)`. 원화 계좌이체뿐 아니라 ATM 현지 지폐 배분·트래블카드 외화 송금도 같은 틀로 들어간다(예산 행과 같은 금액 표현). **이건 지출이 아니라 공금의 이동이라 잔여 예산은 변하지 않는다.** 설정 탭 "💸 공금 나눠주기" 패널에서 입력하고, 정산 결과 텍스트·CSV의 결제자별 항목에 "여행 중 공금에서 미리 받음"으로 근거가 뜬다.
### 18단계에서 확정된 구현 세부사항
- **"예산 초과분 & 개인경비 결제금액" 라벨이 공금 이체 이후로 거짓말이 됐다**: 이 값은 `결제총액 − 본인 개인경비`라 공금을 미리 받아 결제한 사람도 그 전액을 자기가 부담한 것처럼 보였다(율 ₩705,261 — 실제 자기 돈은 ₩5,261). `fundPaid`(그중 공금 결제) / `fundHeld`(선지급된 공금) / `ownPocket`(공금 초과결제분, = fundPaid − fundHeld)을 `PayerSummary`에 추가하고 문구를 바꿨다.
- **총무의 `fundHeld` 에는 나눠주고 남은 예산이 들어간다**: `received + (총무면 budget − handedOut)`. 총무는 따로 이체받지 않아도 남은 공금을 들고 있으므로, 이걸 안 세면 총무가 자기 돈을 훨씬 많이 낸 것처럼 보인다. 이 덕분에 총무 본인 수령을 기록하지 않아도 ₩500,000이 자동으로 잡히고, 수동 계산했던 "혜연 자기부담 ₩289,398"(= 공금 초과결제분)과 정확히 일치한다.
- **총무 미지정이면 기록 자체를 막는다**: 정산식의 예산 항은 "총무가 공금을 들고 있다"는 전제로 세워져 있어서, 지정 없이 기록이 쌓이면 정산이 조용히 어긋난다. 기록 탭에 들어가면 입력 UI 대신 참여자 칩이 뜨고, 한 명을 고르면 바로 지정되며 입력 화면이 나타난다.
- **반올림 보정이 큰 금액을 삼키고 있었다**: 합이 0이 아닐 때 절대값이 가장 큰 사람에게 차액을 몰아주는 코드가 있는데, 총무 미지정이면 예산 항이 아무에게도 안 붙어 **예산 전액(실측 ₩1,199,999)이 한 사람에게 통째로 흡수됐다**. 숫자는 멀쩡해 보이지만 틀린 정산이다. 반올림으로 생길 수 있는 최대 오차는 `인원수 × 1원`이므로, 그보다 크면 경고를 띄운다.
- **총무 항이 `budget` 이 아니라 `budget − Σreceived` 여야 한다**: 나눠준 만큼은 총무 손을 떠났기 때문이다. 이 보정이 없으면 `Σnet = 0` 이 깨진다. 실데이터로 확인: 이체 반영 시 소영/민덩 각 ₩73,665 + 율 ₩68,404 = ₩215,734 가 전부 혜연에게 모이고 순합이 0.
- **`settlement.ts` 는 순수 계산이라 데이터 훅을 import 하면 안 된다**: `receivedByMember` 를 `useFundHandouts` 에 뒀더니 `supabase.ts` → `import.meta.env` 가 딸려와 node 로 계산만 따로 돌려볼 수 없게 됐다. 순수 함수는 `settlement.ts` 안에 둔다.
- **외화로 건넨 경우 환율은 "그 주머니의 환전 환율"** 이어야 한다. 오늘 시세가 아니라 실제로 치른 값이라야 정산이 맞는다(17단계 prepaid 원칙과 같다).
- 이미 빠진(비활성) 참여자에게 나간 이체는 `handedOut` 합계에서 제외한다 — 정산 대상이 아닌 사람 몫까지 총무에게서 빼면 어긋난다.
- 검증은 실제 상하이 데이터(공금 ₩1,494,659 / 예산 ₩1,200,000 / 혜연 ₩789,398·율 ₩705,261 결제)로 5가지 경우를 돌렸다: 이체 없음(기존 회귀), 실제 상황, 일부 반환, 외화 배분, 빈 배열. 전부 순합 0.

### 17단계에서 확정된 구현 세부사항
- **헤더 주머니는 항상 2열**: 통화가 하나면 예전처럼 큰 숫자 한 줄, 둘 이상이면 `grid-template-columns: repeat(2, minmax(0, 1fr))`. 3개면 2+1, 4개면 2+2로 두 줄에 담긴다. 열이 좁아지므로 칸 안에서는 세로로 쌓고 글자를 줄인다(26px → 17px).
- **`.remain span` 이 `.flag` 까지 잡는다**: `.remain span` 에 `width:100%; display:flex` 가 걸려 있어 `<b>` 안에 국기 `<span>` 을 넣으면 국기가 혼자 한 줄을 차지한다. `.remain b .flag { display:inline; width:auto }` 로 되돌려야 한다.
- **원화 칸은 한 줄이 짧다**: 외화 주머니에는 원화 보조줄(`em`)이 있지만 원화 주머니에는 없어서 게이지 높이가 어긋난다. `.pot` 을 flex column 으로 두고 게이지에 `margin-top: auto` 를 줘 바닥에 붙인다.
- 통화별 국기는 `currencies.ts` 의 `FLAGS` 맵(`currencyFlag()`). EUR 는 🇪🇺 처럼 통화권 깃발을 쓴다.
- **외화 예산은 외화 금액이 진짜다.** 원화를 기준으로 삼고 외화를 시세로 역산하면 지갑에 없는 숫자가 나온다. 잔여는 `외화 예산 − 그 통화로 쓴 공금`으로 내고, 원화는 그 주머니 환율로 되돌린 참고값이다.
- **결제건 환율과 환전 환율은 다르다**: 45.38에 환전한 현금으로 1,000 TWD를 쓰면 지갑엔 정확히 14,425 TWD가 남는다. 그런데 그 결제가 카드사 환율 40으로 기록되면(₩40,000) 원화에서 빼는 방식으로는 ₩659,987이 남았다고 나와 지갑과 어긋난다. 외화에서 외화를 빼면 이 오차가 아예 생기지 않는다.
- **주머니가 없는 통화의 공금 지출은 원화 주머니에서 `entries.krw`만큼 뺀다.** 이 폴백이 있어야 예산이 전부 원화인 기존 여행(상하이 등)이 지금까지와 똑같이 동작한다. 이게 없으면 원화 주머니는 영원히 안 줄고 외화 주머니만 음수가 된다.
- **실시간 환율 예산의 `amount`는 스냅샷일 뿐이다.** 입력 당시 시세로 채워 넣되(컬럼이 not null), 화면·합계는 `budgetTotalKrw()`가 그날 시세로 다시 환산한다. 그래서 `useBudgets`는 더 이상 `total`을 내놓지 않고, 호출부가 `rates`와 함께 `budgetTotalKrw(budgets, rates)`를 부른다.
- 여러 번 나눠 환전했으면 주머니 환율은 **가중평균**(Σ 원화 ÷ Σ 외화)을 쓴다.
- 개인 결제는 주머니를 축내지 않는다. 공금(`member_id === null`)만 뺀다.

### 16단계에서 확정된 구현 세부사항
- **`beforeinstallprompt`는 모듈 로드 시점에 잡아야 한다**: 이 이벤트는 페이지가 뜨자마자 한 번 날아오고 다시 오지 않는다. React 컴포넌트 안에서 `useEffect`로 등록하면 이미 지나간 뒤라 영영 못 잡는다. `src/lib/install.ts`가 import 되는 순간 전역에 리스너를 걸어 보관하고, 화면은 `useInstallPrompt()`로 꺼내 쓴다. 이걸 잡아야 안드로이드에서 **OS 설치창을 우리 버튼으로 직접 띄울 수** 있다 — 15단계까지는 아무도 안 잡고 있어서 크롬 기본 배너에만 기대고 있었다.
- **standalone 감지는 두 가지를 다 봐야 한다**: 안드로이드/데스크톱은 `matchMedia('(display-mode: standalone)')`, iOS Safari는 그걸 안 주고 `navigator.standalone`으로만 알린다.
- **iPadOS 13+ 는 UA에 iPad가 없다**: `Macintosh`로 나와서 데스크톱과 구분이 안 된다. `'ontouchend' in document`로 한 번 더 갈라야 iOS 안내가 뜬다.
- 설치/사용법 안내는 **자기 기기 것만** 보여준다. 설정 탭은 아이폰·갤럭시 안내가 항상 둘 다 떠 있어서 자기 것을 골라 읽어야 했다.
- **Playwright `addInitScript`의 함정**: 이 스크립트는 새로고침을 포함한 **매 네비게이션마다** 실행된다. 온보딩 플래그를 여기서 지우거나 심으면, 앱이 방금 바꾼 값을 되돌려놔서 "플래그가 저장이 안 된다"/"버튼이 동작 안 한다"처럼 보인다. 실제로 이 때문에 없는 버그를 두 번 쫓았다. 세션 값(코드·이름)만 `addInitScript`로 두고, 검증 대상 플래그는 `page.evaluate`로 한 번만 심을 것.
- **검증용 접속은 기존 참여자 이름으로**: 15단계 교훈대로 새 `trip_members`가 생기면 `notifyJoin`이 실제 웹푸시를 보낸다. 상하이 여행(`93519374`)의 기존 참여자 "소영"으로 붙어서 알림 없이 검증했다.
- **기존 사용자도 한 번은 본다**: 플래그가 이번에 새로 생겼으므로 이미 쓰고 있던 참여자들도 다음 접속 때 온보딩을 한 번 보게 된다. 설치 안내를 못 봤던 사람들이라 의도한 동작이다.

### 15단계에서 확정된 구현 세부사항
- **최종 정산하기** (`src/lib/settlement.ts`): 설정 탭 "내역 내보내기" 아래 신설. 핵심 공식은 `net(P) = fair(P) - paidRaw(P) - budget/n + (총무면 +budget)` — `fair`는 개인경비+공금n분의1(정당한 몫), `paidRaw`는 `paid_by` 기준 실제 결제액, 마지막 항은 예산 잔여/초과를 총무(모임통장 실물 보유자) 쪽으로 흡수시켜 인당 이체 리스트에 자연스럽게 녹인다. **공금 실사용액(fund)과 예산 총액(budget)을 하나의 항으로 섞으면 안 된다** — 섞으면 `sum(net)`이 0이 안 된다(설계 중 여러 번 겪은 함정, Node 스크립트로 최소 케이스부터 재검증해서 확정). 그리디 최소송금 매칭으로 "누가 누구에게 얼마"를 뽑는다. `trips.treasurer_member_id`(마이그레이션 0012)를 신설해 총무를 지정하며, 미지정이면 예산 잔여/초과는 정산에서 빠지고 경고만 뜬다. "결제자와 개인경비 소유자가 다른 경우"(A가 B 대신 결제)는 이미 있던 `member_id`(몫 귀속)와 `paid_by`(실제 결제자) 조합만으로 자동 반영되고, 새 필드는 필요 없었다. 정산 결과 텍스트/CSV는 `src/lib/settlementExport.ts`, 모달은 `SettlementExportModal`.
- **텍스트 공유는 파일이 아니라 텍스트 자체로**: "내역 내보내기"/"정산내용 내보내기"의 텍스트(카톡용) 버튼이 `.txt` 파일을 공유 시트로 넘기면, 받는 쪽 앱(특히 카톡)이 인코딩을 잘못 짐작해 내용이 깨지는 문제가 있었다. `deliver()`를 `deliverText`(파일 없이 `navigator.share({text})`만 시도, 실패 시 클립보드)와 `deliverFile`(CSV처럼 파일이어야 뜻이 있는 것, 공유 시트를 다운로드보다 먼저 시도)로 분리했다. CSV는 여전히 파일로 공유한다.
- **사진 분석 503 안내 + 연도 없는 날짜 처리**: `parse-image`가 실패 사유(`overloaded`=503/`quota`=429/`other`)를 `reasons` 배열로 함께 반환하도록 확장. 클라이언트는 503 전량 실패 시 "인식 실패"가 아니라 "서버가 붐벼요, 잠시 후 다시 시도하거나 텍스트로 입력해주세요"라고 명확히 안내한다. Gemini가 "8월27일"처럼 연도 없이 월일만 읽으면 그 값을 `monthDay`(MM-DD)로 살려 반환하고, 클라이언트가 업로드 시점(오늘) 연도와 조합해 날짜를 채운다 — 예전엔 연도를 모르면 `date` 전체를 오늘로 덮어써서 월일까지 틀어졌다.
- **Gemini 실패 시 Claude Haiku 4.5로 유료 폴백**: `ANTHROPIC_API_KEY`(Edge Function 시크릿, `GEMINI_API_KEY`와 같은 방식으로 등록)가 있으면, Gemini가 `overloaded`/`quota`로 실패했을 때만 같은 이미지를 Claude Haiku 4.5로 재시도한다. 응답 형식 오류 등(`other`)일 때는 폴백하지 않는다 — 캡처 자체를 못 읽는 문제라 모델을 바꿔도 대개 같은 문제이기 때문. Claude 응답이 가끔 JSON을 마크다운 코드블록으로 감싸는 걸 방어적으로 벗겨낸다. 키가 없으면 폴백 없이 기존 Gemini-only 동작 그대로다. 비용은 이미지 1장당 원 단위(Haiku 4.5 기준)라 무시할 수준. Anthropic API 콘솔은 순수 종량제(pay-as-you-go)로 월 정액/유지비가 없다. 실제로 Gemini가 연속 503을 내던 시점에 curl로 폴백이 정확히 타는 것과(로그: "Gemini failed (overloaded), falling back to Claude") 추출 결과(merchant/amount/monthDay/time 4필드)까지 검증 완료.
- **요약 헤더 표시값 변경**: 최상단 헤더 우측의 `trip.code`(예: `20260903`)가 여행 코드인지 시작일인지 헷갈린다는 지적으로, `trip.start_date ~ trip.end_date`(줄바꿈 가능한 세로 배치)로 바꿨다. `.eyebrow`를 `flex-direction:column`으로 바꾸고 폰트를 줄여 헤더 전체 높이는 그대로 유지했다.
- **설정 탭 꿀팁 버튼 폰트**: `Collapsible` 제목 전부(`.collapse-head`)의 `font-weight`를 600→400으로. 이 클래스를 쓰는 곳이 꿀팁 섹션(및 그 하위 항목)뿐이라 클래스 자체를 고쳐도 범위를 벗어나지 않는다.
- **내역 카드 레이아웃**: 이미 날짜별로 그룹핑돼 있어 항목 안 날짜 표시를 없애고 시간만 남겼다. 첫 줄은 "시간·카테고리·공금/개인"까지, 결제수단부터(결제자·환율·동기화상태)는 항상 둘째 줄로 고정해 항목 길이에 따라 결제자 위치가 들쭉날쭉하던 문제를 없앴다.
- **테스트 데이터 격리**: 이 세션의 여러 검증(정산 계산, 공유 방식, 사진분석)은 전부 별도 테스트 여행(코드 `999900xx`)을 MCP로 만들어 검증 후 삭제하는 방식으로 진행했다. 참여자 등록(`loadMember`)이 새 `trip_members`를 만들면 `notifyJoin`이 실제로 관리자 폰에 웹푸시를 보낸다는 걸 뒤늦게 알았다 — 사용자는 오히려 이걸 파이프라인 실증이라며 긍정적으로 받아들였지만, 다음부터는 검증용 참여자를 MCP로 미리 `trip_members`에 INSERT해두면(기존 참여자로 인식되어 `notifyJoin` 미호출) 불필요한 알림을 피할 수 있다.

### 14단계에서 확정된 구현 세부사항
- **`connectTrip` 이름 유실 버그**: `setStoredTripCode()`가 여행 코드를 저장하면서 `PERSON_NAME_KEY`도 함께 지운다(`src/lib/session.ts`). 그래서 코드를 바꿔 들어갈 때마다("다른 여행으로 이동하기", 관리자 둘러보기) 이미 그 여행의 참여자인데도 이름을 다시 물어봤다. `connectTrip`이 **`setStoredTripCode` 호출 전에** 이름을 읽어두고 이후 되살리도록 고쳤다. 이름은 여행별이 아니라 기기별 저장이라 여행을 옮겨도 같은 사람으로 본다.
- 관리자 비밀번호를 메모리에만 두는 것은 의도된 선택이다. 편의를 위해 `localStorage`에 넣고 싶어지더라도, 그러면 기기를 분실했을 때 남이 여행을 지울 수 있다.
- **Playwright 라우트 목의 함정**: Playwright는 **나중에 등록한 라우트가 먼저 매칭된다**. `**/functions/v1/**` 같은 포괄 목을 뒤에 등록하면 앞서 등록한 `**/admin-trips**`를 덮어써서, 틀린 비밀번호인데도 200이 돌아와 "인증이 뚫린 것처럼" 보인다. 실제로 이 때문에 없는 보안 버그를 한참 쫓았다. 포괄 목에서 `r.fallback()`으로 넘기거나 등록 순서를 뒤집을 것.
- `AdminGate`와 `AdminMenu`가 둘 다 `<h1 className="title">🛠 관리자</h1>`를 렌더한다. 테스트에서 화면을 구분할 때 `.title` 텍스트를 쓰면 안 되고, `#adminPw`의 존재 여부로 판단해야 한다.
...
- [x] Anthropic API 키 발급 — console.anthropic.com, Edge Function 시크릿 `ANTHROPIC_API_KEY`로 등록 완료 (Gemini 503/429 실패 시 Claude Haiku 4.5 폴백용, 순수 종량제라 월 정액 없음)
