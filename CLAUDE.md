...
### 18단계에서 확정된 구현 세부사항
- **"예산 초과분 & 개인경비 결제금액" 라벨이 공금 이체 이후로 거짓말이 됐다**: 이 값은 `결제총액 − 본인 개인경비`라 공금을 미리 받아 결제한 사람도 그 전액을 자기가 부담한 것처럼 보였다(율 ₩705,261 — 실제 자기 돈은 ₩5,261). `fundPaid`(공금 결제액) / `fundHeld`(손에 쥔 공금) / `ownPocket`(= fundPaid − fundHeld)을 `PayerSummary`에 추가하고 문구를 바꿨다.
- **총무의 `fundHeld` 에는 나눠주고 남은 예산이 들어간다**: `received + (총무면 budget − handedOut)`. 총무는 따로 이체받지 않아도 남은 공금을 들고 있으므로, 이걸 안 세면 총무가 자기 돈을 훨씬 많이 낸 것처럼 보인다. 이 덕분에 총무 본인 수령을 기록하지 않아도 ₩500,000이 자동으로 잡히고, 수동 계산했던 "혜연 자기부담 ₩289,398"과 정확히 일치한다.
