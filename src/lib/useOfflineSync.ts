  ...
  for (const queued of ops) {
    try {
      if (queued.op.type === 'insert') {
        await supabase.from('entries').insert({ ...queued.op.item, trip_id: tripId })
      } else if (queued.op.type === 'update') {
        await supabase.from('entries').update(queued.op.patch).eq('id', queued.op.entryId)
      } else if (queued.op.type === 'delete') {
        await supabase.from('entries').delete().eq('id', queued.op.entryId)
      }
 ...
 */
export function useOfflineSync(tripId: string | undefined, refresh: (options?: { clearPending?: boolean }) => void) {
